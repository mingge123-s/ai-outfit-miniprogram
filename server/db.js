...
export const MAX_BACKGROUND_TAGS = 10;

export const backgroundTags = {
  list(userId) {
    return db.prepare("SELECT * FROM background_tags WHERE user_id = ? ORDER BY id DESC").all(userId);
  },
  count(userId) {
    return db.prepare("SELECT COUNT(*) AS c FROM background_tags WHERE user_id = ?").get(userId).c;
  },
  // 返回 { ok, item?, error? }；重复标签直接返回已存在的，达上限报错
  add: db.transaction((userId, text, max = MAX_BACKGROUND_TAGS) => {
    const existing = db.prepare("SELECT * FROM background_tags WHERE user_id = ? AND text = ?").get(userId, text);
    if (existing) return { ok: true, item: existing, duplicated: true };
    const count = db.prepare("SELECT COUNT(*) AS c FROM background_tags WHERE user_id = ?").get(userId).c;
    if (count >= max) return { ok: false, error: `自定义标签已达上限（${max} 个）`, count };
    const info = db.prepare("INSERT INTO background_tags (user_id, text) VALUES (?, ?)").run(userId, text);
    return { ok: true, item: db.prepare("SELECT * FROM background_tags WHERE id = ?").get(info.lastInsertRowid) };
  }),
  remove(userId, id) {
