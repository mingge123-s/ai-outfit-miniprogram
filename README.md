...
`doubao-seed-2-0-mini-260215`，关闭深度思考并使用低细节图片输入。

“今日搭配”使用模糊位置查询实时天气，并根据温度、体感、降雨、湿度、风速和
日常/通勤/约会/运动/旅行场景，从衣柜选择完整组合后直接生成。推荐按日期、
场景和粗粒度位置缓存，避免重复分析；定位不可用时可手动选择寒冷、舒适、炎热或下雨。
启用前需在微信小程序后台申请 `getFuzzyLocation` 接口并更新隐私保护指引。
开发环境默认使用无需密钥的天气接口；商业上线前应确认天气数据服务的商业授权或配置合规套餐。
...
| 权限 | 每日免费生图 | 衣柜上限 | 模特照上限 | 收藏套餐上限 |
| --- | ---: | ---: | ---: | ---: |
| 免费版 | 3 次 | 10 件 | 10 张 | 20 套 |
| 会员 | 10 次 | 30 件 | 30 张 | 40 套 |
...
| 文档 | 说明 |
| --- | --- |
| [交接文档](docs/交接文档.md) | 项目全貌、部署运维、待办事项与故障排查 |
| [运营说明](docs/运营说明.md) | 次数、积分、兑换码、成本核算与数据备份 |
